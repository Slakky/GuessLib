...
<<<<<<< HEAD
            ...
            (proper_formats, form.nr_of_seqs_fout, form.fout_avg_quality,
                form.fin_avg_quality) = check_format_and_remove_low_quality_reads_single(reads_file_name)
            form.save()
            if proper_formats:
                ref = RefGenome.objects.get(name=form.refgenome)
                (form.libtype, form.inwfwd, form.inwrvs,
                    form.collected_pairs, succesful_lib_determination,
                    form.analysis_time) = guesslib_single(ref, reads_file_name)
                Result.objects.filter(pk=form.pk).update(libtype=form.libtype, inwfwd=form.inwfwd, inwrvs=form.inwrvs, collected_pairs=form.collected_pairs, analysis_time=form.analysis_time)
                subject = str(form.title) + ' ' + str(form.date_submit)[0:10]
                from_email = 'GuessLib'
                to = str(user_email)
                html_message = render_to_string('guess/result_email.html', {'Result': Result.objects.get(pk=form.pk), 'Domain': domain, })
...
<<<<<<< HEAD
            ...
            (proper_formats, form.nr_of_seqs_fout, form.fout_avg_quality,
                form.fin_avg_quality) = check_format_and_remove_low_quality_reads_pair(forward_file_name, reverse_file_name)
            form.save()
            if proper_formats:
                ref = RefGenome.objects.get(name=form.refgenome)
                (form.libtype, form.outfwd, form.outrvs, form.inwfwd, form.inwrvs,
                    form.collected_pairs, succesful_lib_determination,
                    form.analysis_time) = guesslib_pair(ref, forward_file_name, reverse_file_name)
                if not form.libtype:
                    subprocess.run(['rm', forward_file_name])
                    subprocess.run(['rm', reverse_file_name])
                else:
                    Result.objects.filter(pk=form.pk).update(libtype=form.libtype, outfwd=form.outfwd, outrvs=form.outrvs, inwfwd=form.inwfwd, inwrvs=form.inwrvs, collected_pairs=form.collected_pairs, analysis_time=form.analysis_time)
